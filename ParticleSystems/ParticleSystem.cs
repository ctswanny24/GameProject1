...
namespace GameProject1
{
    ...
    public abstract class ParticleSystem : DrawableGameComponent
    {
        ...
        public void ResumeParticles(Vector2 velocity, float lifetime)
        {
            for(int i = 0; i < particles.Length; i++)
            {
        ...
        public void SetBoundingCircles(float radius, float offset)
        {
            for(int i = 0; i < particles.Length; i++)
            {
                particles[i].bounds = new BoundingCircle(new Vector2(particles[i].Position.X - offset, particles[i].Position.Y - offset), radius);
            }
        }

        public void UpdateBoundingPosition(float offset)
        {
            for (int i = 0; i < particles.Length; i++)
                particles[i].bounds.Center = new Vector2(particles[i].Position.X - offset, particles[i].Position.Y - offset);
        }
